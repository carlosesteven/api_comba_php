...
) 
{
	...
	if ( !$existencia ) 
	{
		...
		$valida = $db->where('art_isbn', $_REQUEST["art_isbn"])
		->update('articulo', 
				[
		...
		{
			$respuesta["estado"] = "ok";
			$respuesta["detalles"] = "se edito el articulo correctamente";	
		}else{
			$respuesta["estado"] = "error";
			$respuesta["detalles"] = "se produjo un error al intentar editar el articulo";	
		}
	}
}else{
	$respuesta["estado"] = "error";
	$respuesta["detalles"] = "Faltan algunos parametros en la peticion para editar el articulo";	
}
