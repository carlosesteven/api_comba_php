	...
	&& isset( $_REQUEST["user_niv_aca"] ) ) 
{
	...
	if ( !$existencia ) 
	{
		...
		$valida = $db->where('user_cedula', $_REQUEST["user_cedula"])
		->update('usuarios', 
				[
		...
		{
			$respuesta["estado"] = "ok";
			$respuesta["detalles"] = "se edito el usuario correctamente";	
		}else{
			$respuesta["estado"] = "error";
			$respuesta["detalles"] = "se produjo un error al intentar editar el usuario";	
		}
	}
}else{
	$respuesta["estado"] = "error";
